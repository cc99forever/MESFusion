...
# from Swin_transformer_second2flow.FusionBlock import Fusioncls
# from cross_attention_swin_1.FusionBlock import Fusioncls
# from cross_swin_test.FusionBlock import Fusioncls
from cross_attention_swin_4.FusionBlock import Fusioncls
...
# from cross_swin_test.FusionBlock2 import Fusioncls2
from models import *
from models.vision_transformer import FMSwinTransformer
# from models import SOFTNet, SLSwinTransformer
# from models.LGIFormer_soft_net import LGIFormerSOFTNet
...
def generator(X, y, batch_size=36, epochs=1):
    while True:
        for start in range(0, len(X), batch_size):
            ...
            # img2 = np.array(X[start:end])[:, :, :, 3].reshape(num_images, 42, 42, 1)
            # yield [u, v, os, img2], np.array(y[start:end])
    ...
    for index, label in enumerate(y1):
        if (label == 1):  # Only augment on expression samples (label=1)
            for augment_type in range(3):
                ...
                img_transformed = transformations[augment_type](X[index]).reshape(128, 128, 4)
...
def spotting(result, total_gt, final_samples, subject_count, dataset, k, metric_fn, p, show_plot):
    ...
    for videoIndex, video in enumerate(final_samples[subject_count - 1]):
        ...
        if (show_plot):
            ...
            plt.gca().patch.set_alpha(0)  # 设置背景透明
            plt.plot(score_plot_agg, color='b')
            plt.xlabel('Frame')
            plt.ylabel('Score')
...
def training(X, y, groupsLabel, expression_type, final_samples, k, dataset, train, show_plot):
    ...
    for split, (train_index, test_index) in enumerate(logo.split(X, y, groupsLabel)):  # Leave One Subject Out
        ...
        print('------Initializing MESFusion-------')  # To reset the model at every LOSO testing
        ...
        if (train):
            ...
            optimizer = torch.optim.SGD(params=model.parameters(), lr=0.0005)#lr = 0.0005
        ...
        preds, gt, total_gt = spotting(pred, total_gt, final_samples, subject_count, dataset, k, metric_fn, p,
                                       show_plot)
