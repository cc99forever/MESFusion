...
#             # final[:, :, 3] = abs(final[:, :, 3] - final[y61 - 5:y61 + 6, x61 - 5:x61 + 6, 3].mean())
#             # final[:, :, 4] = abs(final[:, :, 4] - final[y61 - 5:y61 + 6, x61 - 5:x61 + 6, 4].mean())
#             # final[:, :, 5] = abs(final[:, :, 5] - final[y61 - 5:y61 + 6, x61 - 5:x61 + 6, 5].mean())
#
#             # Eye masking
...
def extract_preprocess(final_images, k):
    ...
    for video in range(len(final_images)):
        ...
        for img_count in range(final_images[video].shape[0] - k):
            ...
            optical_flow = cv2.optflow.DualTVL1OpticalFlow_create()
            flow = optical_flow.calc(img1, img2, None)
            # img2 = torch.from_numpy(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB))
            # print(img2.shape)
            # print(img2.dtype)
            # print(type(img2))
            magnitude, angle = cv2.cartToPolar(flow[..., 0], flow[..., 1])
