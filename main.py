...
def main(config):
    ...
    with open("SAMMLVME.pkl", "wb") as pkl_file:
        data = {
            "images": images,
            "subjects": subjects,
            "subjectsVideos": subjectsVideos
        }
        _pickle.dump(data, pkl_file)
    #
    # 加载.pkl文件
    with open("SAMMLVME.pkl", "rb") as pkl_file:
        data = _pickle.load(pkl_file)

    # 从恢复的数据中获取images, subjects, subjectsVideos
    images = data["images"]
    subjects = data["subjects"]
    subjectsVideos = data["subjectsVideos"]
    ...
    with open(
        Path(
            dataset_dir,
            f"dataset_{expression_type}_4C_1_features_{image_size}",
        ).with_suffix(".pkl"),
        "wb",
    ) as pkl_file:
...
if __name__ == '__main__':
    ...
    # input parameters
    parser.add_argument('--dataset_name', type=str, default='SAMMLV')  # Specify CASME_sq or SAMMLV only
    ...
    parser.add_argument('--dataset_dir', type=str, default="SAMMLV")
